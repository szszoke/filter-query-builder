...
class Filter<TObject> {
  ...
  field = <TProperty extends keyof TObject>(
    fieldName: TProperty,
  ): FieldFilter<TObject, TObject[TProperty]> =>
    new FieldFilter<TObject, TObject[TProperty]>(this, fieldName as string);

  query = (): IFilterCondition[] => this.conditions;
...
class FieldFilter<TParentObject, TProperty> {
  ...
  ): FieldFilter<TParentObject, TProperty[TNestedField]> =>
    new FieldFilter<TParentObject, TProperty[TNestedField]>(
      this.parent,
